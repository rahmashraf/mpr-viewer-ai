...
class MRIViewer(QWidget):
    ...
    def initUI(self):
        ...
        # 3D Surface Button
        self.surface3d_button = QPushButton("Show 3D Surface", self)
        self.surface3d_button.setCheckable(True)  # Toggle
        self.surface3d_button.clicked.connect(self.toggle_3d_surface)
        self.control_layout.addWidget(self.surface3d_button)
        ...
        # NOW create VTK widget after grid_layout exists
        from vtk.qt.QVTKRenderWindowInteractor import QVTKRenderWindowInteractor
        import vtk

        # VTK widget for 3D surface
        self.vtk_widget = QVTKRenderWindowInteractor(self)
        self.renderer = vtk.vtkRenderer()
        self.vtk_widget.GetRenderWindow().AddRenderer(self.renderer)
        self.renderer.SetBackground(0.1, 0.1, 0.1)
        self.volume_surface = None  # store the vtkActor
        self.vtk_group = QGroupBox("3D Surface View")
        vtk_layout = QVBoxLayout()
        vtk_layout.addWidget(self.vtk_widget)
        self.vtk_group.setLayout(vtk_layout)
        self.grid_layout.addWidget(self.vtk_group, 1, 2)  # 4th viewport
        self.vtk_group.hide()  # initially hidden
    ...
    def toggle_oblique_view(self, checked):
        if checked:
            # Hide 3D surface if active
            if hasattr(self, 'vtk_group') and self.vtk_group.isVisible():
                self.vtk_group.hide()
                self.surface3d_button.setChecked(False)
                self.surface3d_button.setText("Show 3D Surface")
    ...
    def show_3d_surface(self, threshold=300):
        ...
        data = np.nan_to_num(self.scan_array)
        ...
        for v in verts:
            points.InsertNextPoint(v[2], v[1], v[0])  # x,y,z
        ...
        actor.GetProperty().SetColor(1, 1, 1)  # white surface
        ...
        self.volume_surface = actor

    def toggle_3d_surface(self, checked):
        if checked:
            # Hide oblique slice if active
            if hasattr(self, 'oblique_group'):
                self.oblique_group.hide()
            ...
            self.surface3d_button.setText("Hide 3D Surface")
        else:
            self.vtk_group.hide()
            if hasattr(self, 'oblique_group'):
                self.oblique_group.show()
    ...
    def update_axial_slice(self, value):
        ...
        if self.scan_array is not None:
            ...
            self.show_oblique_view()  # update oblique dynamically
