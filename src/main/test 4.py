...
class MRIViewer(QWidget):
    def __init__(self):
        ...
        self.mask_sitk = None
        self.mask_outline_enabled = False
        self.current_orientation = "axial"  # track which plane is active
    ...
    def initUI(self):
        ...
        # --- Mask controls ---
        self.load_mask_button = QPushButton("Load Mask", self)
        self.load_mask_button.clicked.connect(self.load_mask)
        self.control_layout.addWidget(self.load_mask_button)

        self.show_outline_button = QPushButton("Show Surface Outline", self)
        self.show_outline_button.setCheckable(True)
        self.show_outline_button.clicked.connect(self.toggle_mask_outline)
        self.control_layout.addWidget(self.show_outline_button)
        # --- End mask controls ---
    ...
    def update_axial_slice(self, value):
        ...
        self.current_orientation = "axial"  # (or coronal/sagittal accordingly)
        if self.mask_outline_enabled:
            self.update_mask_outline_view()
    ...
    def update_coronal_slice(self, value):
        ...
        if self.scan_array is not None:
            ...
            if self.oblique_enabled:
                self.show_oblique_view()
        self.current_orientation = "coronal"  # (or coronal/sagittal accordingly)
        if self.mask_outline_enabled:
            self.update_mask_outline_view()
    ...
    def update_sagittal_slice(self, value):
        ...
        if self.scan_array is not None:
            ...
            if self.oblique_enabled:
                self.show_oblique_view()
        self.current_orientation = "sagittal"  # (or coronal/sagittal accordingly)
        if self.mask_outline_enabled:
            self.update_mask_outline_view()
    ...
    def load_mask(self):
        """Load segmentation mask file (NIfTI or NumPy)."""
        ...
        try:
            if file_path.endswith(('.nii', '.nii.gz')):
                ...
                self.status_bar.showMessage("Unsupported mask format.")
                return

            if arr.ndim != 3:
                self.status_bar.showMessage("Mask must be 3D.")
                return

            self.mask_array = (arr > 0).astype(np.uint8)
            self.status_bar.showMessage(f"Mask loaded: {file_path}")
        except Exception as e:
            self.status_bar.showMessage(f"Error loading mask: {e}")
            print("Mask load error:", e)

    def toggle_mask_outline(self, checked):
        """Show or hide mask outline in the 4th view."""
        if checked:
            if self.mask_array is None:
                self.status_bar.showMessage("Load a mask first.")
                self.show_outline_button.setChecked(False)
                return

            # Hide oblique if active
            if getattr(self, "oblique_enabled", False):
                self.toggle_oblique_view(False)
                self.oblique_button.setChecked(False)

            self.mask_outline_enabled = True
            self.show_outline_button.setText("Hide Surface Outline")
            ...
            self.show_outline_button.setText("Show Surface Outline")

    def update_mask_outline_view(self):
        """Update the 4th (VTK) view with the contour of the current slice."""
        if not self.mask_outline_enabled or self.mask_array is None:
            return

        # --- choose slice based on current orientation ---
        if self.current_orientation == "axial":
            idx = self.axial_slider.value()
            slice_data = self.mask_array[idx, :, :]
        elif self.current_orientation == "coronal":
            idx = self.coronal_slider.value()
            slice_data = self.mask_array[:, idx, :]
        elif self.current_orientation == "sagittal":
            idx = self.sagittal_slider.value()
            slice_data = self.mask_array[:, :, idx]
        else:
            return
