...
class MRIViewer(QWidget):
    ...
    def on_press(self, event, view_index):
        if event.button == 1 and event.inaxes:  # Left mouse button
            if self.draw_roi_button.isChecked():
                ...
                if view_index == 0: self.axial_ax.roi_patch = patch
                elif view_index == 1: self.coronal_ax.roi_patch = patch
                elif view_index == 2: self.sagittal_ax.roi_patch = patch
    ...
    def on_motion(self, event, view_index):
        if self.drawing_roi and event.inaxes and self.roi_start_pos:
            ...
            patch = None
            if view_index == 0: patch = getattr(self.axial_ax, 'roi_patch', None)
            elif view_index == 1: patch = getattr(self.coronal_ax, 'roi_patch', None)
            elif view_index == 2: patch = getattr(self.sagittal_ax, 'roi_patch', None)
    ...
    def show_coronal_slice(self, scan, slice_index):
        ...
        self.coronal_hline = self.coronal_ax.axhline(self.scan_array.shape[0] - 1 - self.crosshair_z, color='r', linestyle='--')
    ...
    def show_sagittal_slice(self, scan, slice_index):
        ...
        self.sagittal_hline = self.sagittal_ax.axhline(self.scan_array.shape[0] - 1 - self.crosshair_z, color='r', linestyle='--')
    ...
    def show_oblique_view(self):
        ...
        try:
            ...
            if not hasattr(self, 'oblique_fig'):
                ...
                self.oblique_group = self.create_viewport_group("Oblique View", self.oblique_canvas, self.oblique_slider)
